...
module.exports = opts => {
  ...
  _.forEach(opts.models, function(m) {
    ...
    if (m.schema) {
      collection.schema = _.mapValues(m.schema, s =>
        Joi.compile(s).options(joiErrors));
    }
      ...
      _.forEach(m.indexes.splice(0), function(i) {
        ...
        collection.createIndex(i[0],
          ...
          Object.assign({ background: true },
            (i[1] || {})),
          function() {
            opts.logger.info(`Index created @ ${m.name} ${util.inspect(i[0])}`);
          });
      });
    }
